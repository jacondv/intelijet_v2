...
def run_example():
    ...
    logging.info("Stream and visualization started. Close the Open3D-window in order to stop this example.")

    num_visualized_points = 0
    MIN_POINTS_PER_UPDATE = 10000

    # Update the visualization for every MIN_POINTS_PER_UPDATE points
    while True:
        num_world_points = len(stream_handler.world_points.points)
        if num_world_points > num_visualized_points + MIN_POINTS_PER_UPDATE:
            # The geometry should only be added when it contains multiple points
        ...
        # Triggers when the open3d-window is closed
        if not vis.poll_events():
            break
        vis.update_renderer()

        # Stop the scanner if the stream was stopped due to low SLAM-confidence
        if not stream_handler.is_running and blk_arc.is_scanning():
            blk_arc.stop_capture()
        time.sleep(0.1)

    # Stop the stream before stopping the scanner
    stream_handler.stop()
